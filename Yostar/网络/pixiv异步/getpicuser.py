...
from getuserpage import getuserpage

async def getpic(session,picid,tag):
    headers = {
    ...
    async with session.get(picurl,headers=headers) as typejudge:
        ...
        title = re.findall(r'"twitter:title" content="(.*?)"', jtext)[-1]
        if re.search('[\\\ \/ \* \? \" \: \< \> \|]', title) != None:
            title = re.sub('[\\\ \/ \* \? \" \: \< \> \|]', '', title)
        selector = parsel.Selector(jtext)
        con = selector.xpath('//meta[@name="preload-data"]').get()
        tjson = json.loads(re.findall('content=\'(.*)\'', con)[-1])
        pageCount=int(tjson["illust"][f'{id}']["pageCount"])
        if '.jpg' in result:
            type = 'jpg'
        if '.png' in result:
            type = 'png'
...
async def main(uid):
    async with aiohttp.TCPConnector(limit=5) as conn:
        async with aiohttp.ClientSession(connector=conn) as session:
            list,tag=getuserpage(uid)
            if list != []:
                print(f'列表已经到手啦,作者为：{tag}，有{len(list)}页')
